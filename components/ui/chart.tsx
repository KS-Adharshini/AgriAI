...
// Using loose types to maintain compatibility across Recharts versions
...
function ChartTooltipContent(props: any) {
  ...
  const payload = (props as any).payload as AnyPayload
  ...
  const tooltipLabel = React.useMemo(() => {
    if (hideLabel || !castPayload || !castPayload.length) {
      return null
    }

    const [item] = castPayload
    ...
    if (labelFormatter) {
      return (
        <div className={cn("font-medium", labelClassName)}>
          {labelFormatter(value as any, castPayload as any)}
        </div>
      )
    }
  ...
  }, [label, labelFormatter, castPayload, hideLabel, labelClassName, config, labelKey])

  if (!active || !castPayload || !castPayload.length) {
    return null
  }
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-1.5">
        {castPayload.map((item: any, index: number) => {
